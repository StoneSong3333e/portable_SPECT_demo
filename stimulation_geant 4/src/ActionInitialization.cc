...
{
  // 1️⃣ Primary generator
  auto generator = new PrimaryGeneratorAction;
  SetUserAction(generator);

  // 2️⃣ RunAction
  auto runAction = new RunAction;
  runAction->InitEmitMap(generator->GetNx(), generator->GetNy());
  SetUserAction(runAction);
  ...
  // 4️⃣ SteppingAction（依赖 EventAction）
  auto steppingAction = new SteppingAction(eventAction);
  SetUserAction(steppingAction);

  // 5️⃣ generator ← eventAction（你之前已经设计好了）
  generator->SetEventAction(eventAction);
}
